...
def gaussian_3d_coeff(xyzs, covs):
    ...
    power = -0.5 * (x**2 * inv_a + y**2 * inv_d + z**2 * inv_f) - x * y * inv_b - x * z * inv_c - y * z * inv_e # 计算高斯分布的指数部分
...
def build_rotation(r):
    norm = torch.sqrt(r[:,0]*r[:,0] + r[:,1]*r[:,1] + r[:,2]*r[:,2] + r[:,3]*r[:,3]) # 输入四元数r控制椭球的方向,计算四元数的范数

    q = r / norm[:, None] # 归一化四元数
...
def build_scaling_rotation(s, r): #缩放参数s控制椭球在三个主轴上的大小
    ...
    L = R @ L # 旋转矩阵R与缩放矩阵L相乘，得到最终的变换矩阵L
    return L

class BasicPointCloud(NamedTuple):
    points: np.array # 点的3D坐标  [N, 3]形状的数组
    colors: np.array # 点的颜色信息 [N, 3]形状的数组
    normals: np.array # 点的法向量 [N, 3]形状的数组
...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            ...
            symm = strip_symmetric(actual_covariance) # 将对称的3×3协方差矩阵压缩为6元素向量
            return symm
        
        self.scaling_activation = torch.exp # 激活函数
        self.scaling_inverse_activation = torch.log # 逆激活函数

        self.covariance_activation = build_covariance_from_scaling_rotation # 协方差构建函数
    ...
    def __init__(self, sh_degree : int):
        self.active_sh_degree = sh_degree # 指定球谐函数的阶数
        self.max_sh_degree = sh_degree  # 存储最大球谐阶数
        self._xyz = torch.empty(0)
        self._features_dc = torch.empty(0) # 球谐系数的直流分量
        self._features_rest = torch.empty(0) # 球谐系数的高阶分量
        self._scaling = torch.empty(0) # 高斯椭球体在三个主轴方向的缩放因子
        self._rotation = torch.empty(0)
        self._opacity = torch.empty(0)
        self.max_radii2D = torch.empty(0) # 每个高斯点在2D屏幕上的最大半径
        self.xyz_gradient_accum = torch.empty(0) # 位置梯度的累积值
        self.denom = torch.empty(0) # 累积次数计数器
        self.optimizer = None
        self.percent_dense = 0 # 点云密度百分比
    ...
    def extract_fields(self, resolution=128, num_blocks=16, relax_ratio=1.5):
        # resolution: resolution of field 从高斯点云提取网格表面
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float = 1):
        ...
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda()) # 假设输入[N, 3]形状的数组
    ...
    def save_ply(self, path):
        ...
        rgb = np.clip(f_dc * 255, 0, 255).astype(np.uint8)  # 假设f_dc范围为0-1
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()

        # 定义PLY属性，颜色用标准red/green/blue
        dtype_full = [
            ('x', 'f4'), ('y', 'f4'), ('z', 'f4'),
            ('nx', 'f4'), ('ny', 'f4'), ('nz', 'f4'),
...
class Renderer:
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        ...
        if input is None:
            ...
            xyz = np.stack((x, y, z), axis=1) #形成[N, 3]形状的数组
            # xyz = np.random.random((num_pts, 3)) * 2.6 - 1.3

            shs = np.random.random((num_pts, 3)) / 255.0 # 形成[N, 3]形状的数组
            pcd = BasicPointCloud(
                points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)) # 形成[N, 3]形状的数组
            )
